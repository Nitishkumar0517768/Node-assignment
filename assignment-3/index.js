...
app.get("/states/highest-gdp", (req, res) => {
    ...
    const highestGdp = states.reduce((maxGdp, currentGdp) => {
        if (currentGdp.gdp > maxGdp.gdp) {
            return currentGdp;
        } else {
            return maxGdp;
        }
...
app.get("/states/:id", (req, res) => {
    ...
    const oneState = states.find(e => e.id === uid);
...
app.put("/states/:id", (req, res) => {
    const uid = Number(req.params.id);
    const index = states.findIndex(e => e.id === uid);
...
app.put("/states/:id/budget", (req, res) => {
    ...
    res.status(200).json({
        message: 'annualBudget change',
        budget: states[index].annualBudget
    })
})
...
app.put("/states/:id/population", (req, res) => {
    ...
    res.status(200).json({
        message: 'population changed',
        newPopulation: states[index].population
    })
})
...
app.patch("/states/:id/literacy", (req, res) => {
    ...
    res.status(200).json({
        message: "literacy change",
        newLiteracy: states[index].literacyRate
    })
})
...
app.patch("/states/:id", (req, res) => {
    const uid = Number(req.params.id);
    const state = states.find(e => e.id === uid);
...
app.delete("/states/:id", (req, res) => {
    ...
    res.status(204).json({
        ...
        deletedState: states[index] 
    });

})


// 12. delete state/name/:stateName
app.delete("/states/name/:stateName", (req, res) => {
    const stName = req.params.stateName.toLowerCase();
    const index = states.findIndex( e => e.name.toLowerCase() === stName);
    ...
    res.status(204).json({message: 'state delete sucessfully'});
})
...
app.delete("/states/low-literacy/:percentage", (req, res) => {
  ...
  res.status(200).json({
    deletedCount: originalLength - states.length
  });
});
